...
| **Phòng chống Deepfake** | API Hive Moderation + xác thực kênh phụ ngoài luồng (Step Functions) | Phát hiện deepfake giọng nói >80% | Đánh giá tại Sprint 10 |
| **Tấn công chèn lệnh (Prompt injection)** | **API Lakera Guard (v1, Sprint 8)** → Tinh chỉnh BERT (v2, gói Enterprise, Sprint 23–24, chỉ áp dụng nếu chi phí Lakera quá đắt đỏ + có đủ dữ liệu gán nhãn) | TPR >85%, FPR <2% (cho mô hình BERT); đối với Lakera Guard: dựa trên cam kết SLA của nhà cung cấp | Sprint 8 / Sprint 24 |

**Chính sách cổng kiểm định độ chính xác:** Không có tính năng nào thuộc Luồng 2 được phát hành dưới dạng GA (bản chính thức) cho đến khi đáp ứng cổng kiểm định độ chính xác. Nếu không đạt cổng kiểm định → tính năng sẽ giữ nguyên ở trạng thái `beta` (chỉ cho phép đăng ký dùng thử, không áp dụng SLA). Hoạt động phát hành Luồng 1 không bao giờ bị trì hoãn bởi Luồng 2.
...
| **S3** (Tuần 5–6) | Đồng bộ M365 + delta link, dashboard hợp nhất (Google + M365), chỉ số rủi ro trên mỗi người dùng/ứng dụng | Nguyên mẫu phát hiện chèn lệnh v0.1 (fine-tune BERT-tiny, mục tiêu >80% precision) · Thiết lập pipeline biên dịch Presidio WASM · Tài khoản API Lakera Guard + đo lường chi phí/request | Toàn bộ tài sản từ cả hai nhà cung cấp hiển thị · Luồng 2: prototype >80% precision được ghi nhận |
...
| **S5** (Tuần 9–10) | Phát hiện Slack + AWS IAM, RBAC + khuyến nghị đặc quyền tối thiểu, biểu đồ định danh tổng hợp | **Cổng kiểm định độ chính xác 1 (Tuần 10):** Chèn lệnh >90% · DLP Tier 2 BERT-tiny ONNX hoạt động trong trình duyệt · Shadow AI classification >95% trên top-100 công cụ · Hive API live | 4 nhà cung cấp hợp nhất · Luồng 2: Báo cáo cổng kiểm định 1 |
